...
const getQualityWarnings = (domainFeatures, segments, transcription) => {
  if (!domainFeatures.length) return [];

  const warnings = [];
  const textToCheck = transcription && transcription.length > 20 ? transcription : '';
  const detected = textToCheck ? detectFeaturesInText(textToCheck, domainFeatures) : {};

  const segmentNulls = segments
    ? Object.entries(segments)
        .filter(([_, v]) => !v || v === 'Not mentioned' || v === 'null' || v === 'Not Mentioned')
        .map(([k]) => k)
    : [];
...
const CollectorHome = () => {
  ...
  // Quality checks
  const qualityWarnings = getQualityWarnings(domainFeatures, segments, transcription);
  ...
  useEffect(() => {
    ...
    const fetchTask = async () => {
      try {
        ...
        setDomainFeatures(data.features || []);
      } catch (err) {
  ...
  const startRecording = async () => {
    try {
      ...
      setSegments({}); // reset previous segments
    } catch (err) {
      console.error(err);
  ...
  const uploadAudioToServer = async (blob) => {
    try {
      ...
      if (response.ok) {
        ...
        // ✅ Correctly set segments from backend
        if (data.extracted_data || data.segments || data.features) {
          setSegments(data.extracted_data || data.segments || data.features || {});
        }

        alert(`✅ Engine Match Success!\nProgress: ${data.progress?.percent || 0}% of domain goal target.`);
      } else {
    ...
    <div className="collector-focus-mode">
      ...
      <div className={`status-bar ${!isOnline ? 'offline' : ''}`}>
        <div className="status-item">
          TASK: <strong>{task || 'General Collection'}</strong>
        </div>
        <div className="status-item">
          REF: <code className="ref-code">{ref}</code>
        </div>
        ...
        <div className="status-indicator">
          <span
            className={`dot ${
              !isOnline ? 'yellow' : isRecording ? (isPaused ? 'yellow' : 'red') : 'green'
            }`}
          ></span>
      ...
      <div className="engine-container">
        ...
        {qualityWarnings.length > 0 && (
          <div
            style={{
              background: qualityBlocked ? '#fef2f2' : '#fffbeb',
              border: `1px solid ${qualityBlocked ? '#fecaca' : '#fde68a'}`,
              borderRadius: 12,
              padding: '12px 16px',
              marginBottom: 16,
            }}
          >
            {qualityWarnings.map((w, i) => (
              <div key={i} style={{ fontSize: '0.85rem', color: qualityBlocked ? '#b91c1c' : '#92400e' }}>
                ⚠️ {w.message}
                {w.fields?.length > 0 && (
        ...
        {Object.keys(segments).length > 0 && (
          <div style={{ marginTop: 16, padding: '12px', background: '#f8fafc', borderRadius: 10 }}>
            <strong style={{ fontSize: '0.8rem', color: '#334155' }}>Extracted Features:</strong>
            <pre style={{ fontSize: '0.75rem', marginTop: 6, whiteSpace: 'pre-wrap' }}>
        ...
        <div className="action-footer">
          ...
          <button
            className="save-btn"
            disabled={isRecording || duration === 0 || qualityBlocked}
            onClick={() => navigate('/userDashboard')}
          >
            {qualityBlocked ? 'Fix Missing Features' : 'Return to Dashboard'}
          </button>
        </div>
      </div>
    </div>
